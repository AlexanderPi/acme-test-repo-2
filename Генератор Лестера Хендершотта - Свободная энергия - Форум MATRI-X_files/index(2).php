...
{var shareKey=elem.id.replace(/slink_/,'');var ok=false;var callback=null;var h='190';if(shareKey=='twitter')
{if(ipb.vars['member_id']&&ipb.vars['twitter_id'])
{callback=ipb.sharelinks.twitterPostPop;ok=true;h='110';}}
...
$('cLeft').update(newTotal);}};ipb.sharelinks.init();;ipsTextArea={insertAtCursor:function(editorId,text)
...
IPSCKTools={getSelectionHtml:function(editor)
...
ipb.textEditor.lastSetUp=editorId;},bindHtmlCheckbox:function(elem,editorId)
{editorId=(editorId)?editorId:ipb.textEditor.getEditor().editorId;if($(elem))
{ipb.textEditor.htmlCheckbox[editorId]=$(elem);$(elem).writeAttribute('data-editorId',editorId);$(elem).observe('change',ipb.textEditor.htmlModeToggled.bindAsEventListener(this,$(elem)));}},htmlModeToggled:function(e,elem)
{textObj=ipb.textEditor.getEditor($(elem).readAttribute('data-editorId'));isRte=textObj.isRte();button=$('cke_'+textObj.editorId).down('.cke_button_ipsswitch');if(elem.checked)
{$('editor_html_message_'+textObj.editorId).show();if(isRte)
{button.hide();ipb.textEditor.switchEditor(true,textObj.editorId);}
else
{button.hide();}}
else
{$('editor_html_message_'+textObj.editorId).hide();if(!button.visible())
{button.show();}}},switchEditor:function(noSaveChange,editorId)
{editorId=(editorId)?editorId:ipb.textEditor.getEditor().editorId;textObj=ipb.textEditor.getEditor(editorId);isRte=textObj.isRte();ourcontent=textObj.getText();saveCookie=(noSaveChange)?false:true;htmlOps=null;noSmilies=0;if($('noSmilies_'+editorId))
...
var _url=ipb.textEditor.ajaxUrl+'&app=core&module=ajax&section=editor&do=switch&secure_key='+ipb.vars['secure_hash'];if(htmlOps!==null)
{_url+='&htmlStatus='+htmlOps;}
Debug.write(_url);Debug.write('Fetched editor content: '+ourcontent);new Ajax.Request(_url,{method:'post',parameters:{'content':ourcontent.encodeParam(),'isRte':isRte,'noSmilies':noSmilies},onSuccess:function(t)
{if(t.responseText!='nopermission')
{Debug.write('Converted to: '+t.responseText);if(textObj.isRte())
{textObj.setIsRte(false,noSaveChange);textObj.CKEditor.setMode('source');$(textObj.editorId).value=ipb.textEditor.stdPre(t.responseText);try
{Debug.write($F(textObj.editorId));textObj.CKEditor.setData($F(textObj.editorId));}
catch(err)
...
{$('cke_'+textObj.editorId+'_stray').remove();if($('ips_x_smile_show_all'))
{$('ips_x_smile_show_all').remove();}}}
else
{textObj.setIsRte(true,noSaveChange);try
{textObj.CKEditor.setData(ipb.textEditor.ckPre(t.responseText));}
catch(err)
{Debug.write("CKEditor error: "+err);}
textObj.CKEditor.setMode('wysiwyg');if(saveCookie)
{ipb.Cookie.set('rteStatus','rte');}}}}.bind(this),onException:function(t)
{Debug.dir(t);}});},toggleEditors:function(mode)
{return false;ipb.textEditor.mainStore.each(function(instance){if(instance.key!=ipb.textEditor.lastSetUp)
{_editor=ipb.textEditor.getEditor(instance.key);if(!_editor.options.isHtml)
{if(mode=='std')
{instance.value.setIsRte(false);instance.value.CKEditor.setMode('source');if($('cke_'+_editor.editorId+'_stray'))
{$('cke_'+_editor.editorId+'_stray').remove();if($('ips_x_smile_show_all'))
{$('ips_x_smile_show_all').remove();}}}
else
{instance.value.setIsRte(true);instance.value.CKEditor.setMode('wysiwyg');}}}});},getEditor:function(editorId)
...
{return ipb.textEditor.lastSetUp;}}},ckPre:function(text)
{text=text.replace('<script','&#60;script');text=text.replace('</script','&#60;/script');if(text.charAt(0)!='<')
{}
return text;},stdPre:function(text)
...
{}
return text;},pad:function(n)
{return("0"+n).slice(-2);}};IPBoard.prototype.textEditorObjects=Class.create({editorId:{},popups:[],cookie:'rte',timers:{},options:{},CKEditor:null,EditorObj:null,initialize:function(editorId,options)
{this.editorId=editorId;this.cookie=ipb.Cookie.get('rteStatus');this.options=Object.extend({type:'full',ips_AutoSaveKey:false,height:0,minimize:0,minimizeNowOpen:0,isRte:1,isHtml:0,bypassCKEditor:0,isTypingCallBack:false,delayInit:false,noSmilies:false,ips_AutoSaveData:{},ips_AutoSaveTemplate:new Template(ipb.textEditor.IPS_AUTOSAVETEMPLATE)},arguments[1]);if(this.options.type=='ipsacp')
{this.setIsRte(true);}
...
{this.setIsRte(0);this.options.bypassCKEditor=1;}
if(!this.options.delayInit)
{this.initEditor();}},setIsRte:function(value,noSaveChange)
...
return val;},initEditor:function(initContent)
...
try
{var config={toolbar:(this.options.type=='ipsacp')?'ipsacp':(this.options.type=='mini'?'ipsmini':'ipsfull'),height:(Object.isNumber(this.options.height)&&this.options.height>0)?this.options.height:(this.options.type=='mini'?150:300),ips_AutoSaveKey:this.options.ips_AutoSaveKey};if(!this.isRte())
...
{Debug.write('CKEditor error: '+err);}
this.CKEditor=CKEDITOR.instances[this.editorId];ipb.textEditor._tmpContent=(Object.isString(initContent)||Object.isNumber(initContent))?initContent:'';CKEDITOR.on('instanceReady',function(ev)
{if(ev.editor.name==this.editorId)
{this.EditorObj=ev;if(this.options.minimize)
{try
{$('cke_top_'+this.editorId).down('.cke_toolbox_collapser').hide();$('cke_bottom_'+this.editorId).hide();$('cke_'+this.editorId).down('.cke_wrapper').addClassName('minimized');if(!this.isRte())
...
{this.showEditor().bind(this);}}catch(e){}}.bind(this));}
...
{$('cke_'+this.editorId).insert(new Element('input',{type:'checkbox',id:'cbx_'+this.editorId,checked:true}).hide());ipb.textEditor.bindHtmlCheckbox($('cbx_'+this.editorId),this.editorId);}
...
{$('cke_top_'+this.editorId).down('.cke_button_ipsemoticon').up('.cke_button').hide();}}}.bind(this));},showEditor:function(content)
{if(this.options.minimize)
{this.options.minimizeNowOpen=1;$('cke_top_'+this.editorId).down('.cke_toolbox_collapser').show();$('cke_bottom_'+this.editorId).show();$('cke_'+this.editorId).down('.cke_wrapper').removeClassName('minimized');this.EditorObj.editor.execCommand('toolbarCollapse');if(Prototype.Browser.IE9)
...
{this.options.isTypingCallBack();clearInterval(this.timers['dirty']);this.timers['dirty']='';}},minimizeOpenedEditor:function()
...
{$('ips_x_smile_show_all').remove();}}
this.EditorObj.editor.execCommand('toolbarCollapse');$('cke_top_'+this.editorId).down('.cke_toolbox_collapser').hide();$('cke_bottom_'+this.editorId).hide();$('cke_'+this.editorId).down('.cke_wrapper').addClassName('minimized');if(this.isRte())
{this.EditorObj.editor.setData('<p></p>');this.EditorObj.editor.focusManager.forceBlur();}
else
{this.EditorObj.editor.setData('');try
...
catch(e){Debug.error(e);}}},scrollTo:function()
...
{this.CKEditor.destroy(true);this.CKEditor=null;ipb.textEditor.mainStore.unset(this.editorId);if(!Object.isUndefined($H(this.timers)))
{$H(this.timers).each(function(timer)
{var name=timer.key;if(name.match(/^interval_/))
{clearInterval(this.timers[name]);Debug.write('Cleared Interval '+name);}
else
...
{$(this.editorId).value+=content;}},displayAutoSaveData:function()
{if(inACP)
{return;}
try
...
{setTimeout(function(){this.CKEditor.setData(this.options.ips_AutoSaveData.restore_std);}.bind(this),500);}},save:function(editor,command)
{if(inACP)
{return;}
...
{var s='__last_update_stamp_'+this.editorId;var d=new Date();var c=new Template(ipb.textEditor.IPS_SAVED_MSG).evaluate({time:d.toLocaleTimeString()});$$('.'+s).invoke('remove');$('cke_'+this.editorId).down('.cke_resizer').insert({before:new Element('div').addClassName('cke_path '+s).update(c)});ipb.delegate.register('._as_explain',this.launchExplain.bind(this));}});;var _topic=window.IPBoard;_topic.prototype.topic={totalChecked:0,inSection:'',postcache:[],poll:[],pollPopups:[],deletePerms:{},deleteUrls:{},deletePopUps:[],hidePopUps:[],restorePopUps:[],counts:{},timers:{},polling:{count:0},mqBoxShowing:false,fastReplyId:'',isPosting:false,modOptionsUnapproved:0,modOptionsHidden:0,modOptionsUnhidden:0,init:function()
